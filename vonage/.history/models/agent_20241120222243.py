...
class VCCAgent(models.Model):
    ...
    def parse_users(self, users):
        ...
        for user in users:
            print(f"USER ###### {user}")
            username = user.get("username")  # Ensure this is unique for each user
            ...
            if agent_record is not None:
                ...
                agent_record.write({"username": username, "name": name, "email": email})
            else:
                print(f"Creating new agent: {username}")
                self.env["vcc.agent"].create(
                    {"agent_id": username, "name": name, "email": email}
                )
